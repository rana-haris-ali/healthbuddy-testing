export const REGISTER_DOCTOR_REQUEST = 'REGISTER_DOCTOR_REQUEST';
export const REGISTER_DOCTOR_SUCCESS = 'REGISTER_DOCTOR_SUCCESS';
export const REGISTER_DOCTOR_FAILURE = 'REGISTER_DOCTOR_FAILURE';

export const DOCTORS_LIST_REQUEST = 'DOCTORS_LIST_REQUEST';
export const DOCTORS_LIST_SUCCESS = 'DOCTORS_LIST_SUCCESS';
export const DOCTORS_LIST_FAILURE = 'DOCTORS_LIST_FAILURE';

export const GET_SINGLE_DOCTOR_REQUEST = 'GET_SINGLE_DOCTOR_REQUEST';
export const GET_SINGLE_DOCTOR_SUCCESS = 'GET_SINGLE_DOCTOR_SUCCESS';
export const GET_SINGLE_DOCTOR_FAILURE = 'GET_SINGLE_DOCTOR_FAILURE';

export const PATIENTS_LIST_REQUEST = 'PATIENTS_LIST_REQUEST';
export const PATIENTS_LIST_SUCCESS = 'PATIENTS_LIST_SUCCESS';
export const PATIENTS_LIST_FAILURE = 'PATIENTS_LIST_FAILURE';

export const PATIENT_REQUEST_ACCEPTANCE_REQUEST =
	'PATIENT_REQUEST_ACCEPTANCE_REQUEST';
export const PATIENT_REQUEST_ACCEPTANCE_SUCCESS =
	'PATIENT_REQUEST_ACCEPTANCE_SUCCESS';
export const PATIENT_REQUEST_ACCEPTANCE_FAILURE =
	'PATIENT_REQUEST_ACCEPTANCE_FAILURE';
export const PATIENT_REQUEST_ACCEPTANCE_RESET =
	'PATIENT_REQUEST_ACCEPTANCE_RESET';

export const GET_DOCTOR_PROFESSIONAL_INFO_REQUEST =
	'GET_DOCTOR_PROFESSIONAL_INFO_REQUEST';
export const GET_DOCTOR_PROFESSIONAL_INFO_SUCCESS =
	'GET_DOCTOR_PROFESSIONAL_INFO_SUCCESS';
export const GET_DOCTOR_PROFESSIONAL_INFO_FAILURE =
	'GET_DOCTOR_PROFESSIONAL_INFO_FAILURE';
export const GET_DOCTOR_PROFESSIONAL_INFO_RESET =
	'GET_DOCTOR_PROFESSIONAL_INFO_RESET';

export const UPDATE_DOCTOR_PROFESSIONAL_INFO_REQUEST =
	'UPDATE_DOCTOR_PROFESSIONAL_INFO_REQUEST';
export const UPDATE_DOCTOR_PROFESSIONAL_INFO_SUCCESS =
	'UPDATE_DOCTOR_PROFESSIONAL_INFO_SUCCESS';
export const UPDATE_DOCTOR_PROFESSIONAL_INFO_FAILURE =
	'UPDATE_DOCTOR_PROFESSIONAL_INFO_FAILURE';
export const UPDATE_DOCTOR_PROFESSIONAL_INFO_RESET =
	'UPDATE_DOCTOR_PROFESSIONAL_INFO_RESET';

export const DOCTOR_CREATE_REVIEW_REQUEST = 'DOCTOR_CREATE_REVIEW_REQUEST';
export const DOCTOR_CREATE_REVIEW_SUCCESS = 'DOCTOR_CREATE_REVIEW_SUCCESS';
export const DOCTOR_CREATE_REVIEW_FAILURE = 'DOCTOR_CREATE_REVIEW_FAILURE';
export const DOCTOR_CREATE_REVIEW_RESET = 'DOCTOR_CREATE_REVIEW_RESET';

export const DOCTOR_TOGGLE_VERIFICATION_REQUEST =
	'DOCTOR_TOGGLE_VERIFICATION_REQUEST';
export const DOCTOR_TOGGLE_VERIFICATION_SUCCESS =
	'DOCTOR_TOGGLE_VERIFICATION_SUCCESS';
export const DOCTOR_TOGGLE_VERIFICATION_FAILURE =
	'DOCTOR_TOGGLE_VERIFICATION_FAILURE';
export const DOCTOR_TOGGLE_VERIFICATION_RESET =
	'DOCTOR_TOGGLE_VERIFICATION_RESET';
